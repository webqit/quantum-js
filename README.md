...
Right in how the instructions in an imperative program act on data - from the assignment expression that sets or changes the data held in a local variable (`count = 10`) to the `delete` operator that mutates some object property (`delete object.value`), or the "if" construct that determines the program's execution path based on a certain value - you can see "state" (data) and "effect" (instruction) at play!
