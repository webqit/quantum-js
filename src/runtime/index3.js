...
export { LiveMode, Runtime }

export function $eval( sourceType, parseCompileCallback, source, params ) {
    // params could have: env, functionParams, parserParams, transformerParams, runtimeParams
    ...
    // Proceed to parse-transform
    transformerParams.sourceType = sourceType;
    parserParams.inBrowser = inBrowser;
    transformerParams.base64 = asyncEval && inBrowser && `export default async function(%0) {%1}`;
    const compilation = parseCompileCallback( source, { parserParams, transformerParams } );
    
    // Ensure Promise is only returned for async source types
    if ( compilation instanceof Promise && ![ 'async-function', 'async-script', 'module' ].includes( sourceType ) ) {
        throw new Error( `Parse-transform can only return a Promise for sourceTypes: async-function, async-script, module.` );
    }
    ...
    return _await( compilation, compilation => {
        ...
        const $eval = ( $qIdentifier, source ) => {
            ...
            if ( transformerParams.base64 ) { /* @experimental */
        ...
        return _await( $eval( compilation.identifier + '', compilation ), main => {
            const createRuntime = ( thisContext, $env = env ) => {
                ...
                // Then "this" for function scope
                if ( typeof thisContext !== 'undefined' ) { scope = new Scope( scope, 'this', { [ 'this' ]: thisContext } ); }
                
                return new Runtime( undefined, contextType, { ...runtimeParams, originalSource: compilation.originalSource, executionMode: compilation.isLiveProgram && 'LiveProgram' || 'RegularProgram' }, scope, $main );
            };
